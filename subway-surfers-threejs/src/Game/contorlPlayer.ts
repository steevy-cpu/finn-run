...
export class ControlPlayer extends EventEmitter {
    ...
    startGame(currentAction: string, model: THREE.Group) {
        ...
        // Pre-game the character mirrors the player's pose (CV mimic layer)
        // instead of playing the intro dance clip.
        if (currentAction !== playerStatus.DANCE) {
    ...
    // Only the newest ControlPlayer may own the key handler: instances are
    // recreated on every restart, and stacked listeners would run N restarts
    // (N scene clears, N stacked road sections, N spawns) per key press.
    static keyHandler: ((e: KeyboardEvent) => void) | null = null;
    // @ts-ignore
    addAnimationListener() {
        if (ControlPlayer.keyHandler) {
            window.removeEventListener('keydown', ControlPlayer.keyHandler);
        }
        ControlPlayer.keyHandler = (e: KeyboardEvent) => {
            const key = e.key;
            // 开始游戏
            if (key === 'p') {
                if (!this.gameStart) {
                ...
                if (this.status !== playerStatus.JUMP
                    ...
                    // Input buffer: a jump pressed a moment before landing
                ...
                if (this.way === 1) {
                    ...
                    this.emit('collision');
                    showToast('Hit an obstacle! Watch out!');
                    setTimeout(() => {
                        this.runlookback = false;
                    }, 1040);
                    this.smallMistake += 1;
...
handleLeftRightMove() {
    ...
    // A jump pressed while airborne executes on landing (within 300ms).
    tryQueuedJump() {
        if (this.jumpQueuedAt < 0) {
            return;
        }
        if (performance.now() - this.jumpQueuedAt > 300) {
            this.jumpQueuedAt = -1;
            return;
        }
        if (this.gameStart && this.downCollide
    ...
    // Count a coin exactly once: remove it from the scene on pickup so no
    // code path can ever see (and re-count) it again.
    collectCoin(hit: {object: THREE.Object3D}) {
    ...
    // 2-3 units early and off to the side, inflating the counter.)
    collectNearbyCoins() {
        const nowPlane = Math.floor(this.playerRunDistance / roadLength);
        const p = this.model.position;
        const tmp = new THREE.Vector3();
        const picked: THREE.Object3D[] = [];
        for (const group of [this.environement.coin?.[nowPlane], this.environement.coin?.[nowPlane + 1]]) {
            group?.traverse((m: any) => {
                if (m.name !== 'coin' || !m.isMesh) {
                    return;
                }
                m.getWorldPosition(tmp);
                const dy = tmp.y - p.y;
                if (Math.abs(tmp.x - p.x) < roadWidth / 6 // own lane only
                    && Math.abs(tmp.z - p.z) < 1.2
                    && dy > -0.5 && dy < 2.2) { // jumping clears over coins
                    picked.push(m);
                }
            });
        }
        // Removal happens after traverse — mutating mid-traversal is unsafe.
        for (const m of picked) {
            this.collectCoin({object: m});
        }
    }
    // 控制人物的动作变化
    ...
    checkPlayerDistance() {
        ...
        if (runToLength > 0.45 && this.currentPlane !== nowPlane) {
            ...
            // nowPlane here is 1-based; drop everything behind the previous one.
            this.environement.pruneBehind(nowPlane - 1);
        }
    }
    // 向前的碰撞检测判定
