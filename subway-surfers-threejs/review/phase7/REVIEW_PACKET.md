...
**Mismatches, reported honestly:** (1) from the contact onward both characters are framed from the knees up, so feet are out of the clip's own frame — the brief asked for full bodies; (2) Finn's dorsal fin is visible only during the opening jog; (3) the shot is a near-static three-quarter side view rather than a tracking shot. None of these are caused by the overlay. Captures `catch/catch-overlay-*.png` show the current pane-filling crop.
...
- Overlay `#cv-catch`: DOM `<video muted playsinline preload="none">` over the game pane only; the camera panel and overlay canvas are untouched. Per Steeve's review (2026-09-13) the clip fills the pane like the game does (`object-fit: cover`, anchored at 48 % so both faces and the shoulder contact stay inside the crop) instead of letterboxing the 16:9 frame; the clip's outer ~27 % on each side is cropped at the 1440×900 pane (~25 % at 1920×1080). A native portrait/4:5 clip would remove the crop entirely; that is Steeve's call. Staged once at startup when both flags are on (`preload="auto"` + `load()`), never reloaded per run.
