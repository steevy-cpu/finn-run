...
// Game start is deferred: cv-control/game-control.js calls window.__startGame()
// after pose calibration completes (or via the keyboard-only fallback).
window.__startGame = main;
...
`;
  ...
  function render(now) {
        ...
        if (player.pos[0] == trainF[i].pos[0]) {
          if (player.pos[1] >= trainF[i].pos[1] - 4 && player.pos[1] <= trainF[i].pos[1] + 4) {
            if (player.pos[2] >= trainF[i].pos[2] - 18 && player.pos[2] <= trainF[i].pos[2]) {
              ...
              Die();
              gameOver("YOU LOST", score, coins_collected);
            }
          }
        }
      }
    ...
    if (player.pos[2] <= -800) {
      score = -player.pos[2] + coins_collected;
      gameOver("YOU WON", score, coins_collected);
    }
